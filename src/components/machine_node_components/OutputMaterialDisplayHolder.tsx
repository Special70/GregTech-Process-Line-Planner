...
type MaterialDisplayHolderProps = {
    ingredientData: OutputIngredientData
    nodeId: string
}

export function OutputMaterialDisplayHolder(props: MaterialDisplayHolderProps) {
    ...
    return <>
        <div className="relative inline-flex group">
            <button className="w-17 h-17 bg-white border-2 nodrag" onClick={() => {
                toggleViewForMaterialSuggester(props.ingredientData.id);
                submitDataDetails(props.nodeId, "output", props.ingredientData.id, props.ingredientData.name, props.ingredientData.img_filename, props.ingredientData.amount, "0");
